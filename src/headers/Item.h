...
public:
    ...
    Item(int x, int y, const std::shared_ptr<sf::Texture>& t) : Item(x, y) {setTexture(t);};
     ...
     */
    inline bool isOnScreen() const {if (x > -SCREEN_LOAD_OFFSET and x < (SCREEN_WIDTH + SCREEN_LOAD_OFFSET)) return true; else return false;}

    /**
     * Metoda zwracająca informację, czy element znajduje się w otoczeniu punktu.
     * @param ix położenie na osi poziomej
     */
    inline bool isAround(float ix) const {if (x > ix -20 and x < ix +20 ) return true; else return false;}
     ...
     */
    inline bool isNearbyX(const Item& other) const {
